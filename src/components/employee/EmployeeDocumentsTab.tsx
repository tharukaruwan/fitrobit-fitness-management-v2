...
export function EmployeeDocumentsTab({ memberId, memberName }: EmployeeDocumentsTabProps) {
  const { toast } = useToast();
  const [documents, setDocuments] = useState<EmployeeDocument[]>(sampleDocuments);
  ...
  const handleUpload = () => {
    ...
    const newDoc: EmployeeDocument = {
      id: `doc-${Date.now()}`,
      name: formData.name,
      type: formData.type as EmployeeDocument["type"],
  ...
  const columns: Column<EmployeeDocument>[] = [
    {
      ...
      label: "Document",
      priority: "always",
      render: (val: string, item: EmployeeDocument) => (
